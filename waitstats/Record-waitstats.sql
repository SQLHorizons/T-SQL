...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Collect waitstats', 
		...
		@command=N'USE [SQLOPs]
		  GO
		  IF OBJECT_ID (''olah.tbl_wait_stats'', ''table'') IS NULL
			BEGIN
			  CREATE TABLE [SQLOPs].[olah].[tbl_wait_stats](
