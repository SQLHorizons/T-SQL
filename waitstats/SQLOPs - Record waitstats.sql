/****** Object:  Job [SQLOPs - Record waitstats]    Script Date: 20/03/2017 10:19:55 ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 20/03/2017 10:19:55 ******/
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Collect waitstats]    Script Date: 20/03/2017 10:19:55 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Collect waitstats', 
		...
		@command=N'USE [tempdb]
		  ...
		  IF OBJECT_ID (''dbo.tbltmp_wait_stats'', ''table'') IS NULL
			...
			END;
		  GO
		  INSERT INTO [tempdb].[dbo].[tbltmp_wait_stats]
			SELECT @@SERVERNAME AS ServerName, GETDATE()AS date, * FROM sys.dm_os_wait_stats
			  WHERE wait_time_ms > 0;
		  GO
		  DELETE FROM [tempdb].[dbo].[tbltmp_wait_stats]
