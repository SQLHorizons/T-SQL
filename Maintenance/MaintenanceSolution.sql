...
USE [SQLOPs] -- Specify the database in which the objects will be created.
GO

IF SCHEMA_ID('olah') IS NULL
	EXECUTE('CREATE SCHEMA [olah] AUTHORIZATION [dbo]')
GO
...
SET @BackupDirectory     = N'T:\Data_X\backup_01\SQL2014\MSSQL12.MSSQLSERVER\MSSQL\Backup' -- Specify the backup root directory.
...
IF OBJECT_ID('[olah].[DatabaseBackup]') IS NOT NULL DROP PROCEDURE [olah].[DatabaseBackup]
IF OBJECT_ID('[olah].[DatabaseIntegrityCheck]') IS NOT NULL DROP PROCEDURE [olah].[DatabaseIntegrityCheck]
IF OBJECT_ID('[olah].[IndexOptimize]') IS NOT NULL DROP PROCEDURE [olah].[IndexOptimize]
IF OBJECT_ID('[olah].[CommandExecute]') IS NOT NULL DROP PROCEDURE [olah].[CommandExecute]

IF OBJECT_ID('[olah].[CommandLog]') IS NULL AND OBJECT_ID('[olah].[PK_CommandLog]') IS NULL
BEGIN
CREATE TABLE [olah].[CommandLog](
...
GO
CREATE PROCEDURE [olah].[CommandExecute]
...
BEGIN
  ...
  IF @LogToTable = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'olah' AND objects.[name] = 'CommandLog')
  BEGIN
  ...
  IF @LogToTable = 'Y'
  BEGIN
    INSERT INTO olah.CommandLog (DatabaseName, SchemaName, ObjectName, ObjectType, IndexName, IndexType, StatisticsName, PartitionNumber, ExtendedInfo, CommandType, Command, StartTime)
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [olah].[DatabaseBackup]
...
BEGIN
  ...
  IF NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'olah' AND objects.[name] = 'CommandExecute')
  BEGIN
  ...
  IF EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'olah' AND objects.[name] = 'CommandExecute' AND (OBJECT_DEFINITION(objects.[object_id]) NOT LIKE '%@LogToTable%' OR OBJECT_DEFINITION(objects.[object_id]) LIKE '%LOCK_TIMEOUT%'))
  BEGIN
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        EXECUTE @CurrentCommandOutput01 = [olah].[CommandExecute] @Command = @CurrentCommand01, @CommandType = @CurrentCommandType01, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
        SET @Error = @@ERROR
      ...
      BEGIN
        ...
        BEGIN
          ...
          EXECUTE @CurrentCommandOutput02 = [olah].[CommandExecute] @Command = @CurrentCommand02, @CommandType = @CurrentCommandType02, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
      ...
      BEGIN
        ...
        EXECUTE @CurrentCommandOutput03 = [olah].[CommandExecute] @Command = @CurrentCommand03, @CommandType = @CurrentCommandType03, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
        SET @Error = @@ERROR
      ...
      WHILE EXISTS (SELECT * FROM @CurrentBackupSet WHERE VerifyCompleted = 0)
        BEGIN
          ...
          EXECUTE @CurrentCommandOutput04 = [olah].[CommandExecute] @Command = @CurrentCommand04, @CommandType = @CurrentCommandType04, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
      ...
      BEGIN
        ...
        BEGIN
          ...
          EXECUTE @CurrentCommandOutput05 = [olah].[CommandExecute] @Command = @CurrentCommand05, @CommandType = @CurrentCommandType05, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [olah].[DatabaseIntegrityCheck]
  ...
  BEGIN
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            EXECUTE @CurrentCommandOutput08 = [olah].[CommandExecute] @Command = @CurrentCommand08, @CommandType = @CurrentCommandType08, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @SchemaName = @CurrentSchemaName, @ObjectName = @CurrentObjectName, @ObjectType = @CurrentObjectType, @LogToTable = @LogToTable, @Execute = @Execute
            SET @Error = @@ERROR
      ...
      BEGIN
        ...
        EXECUTE @CurrentCommandOutput09 = [olah].[CommandExecute] @Command = @CurrentCommand09, @CommandType = @CurrentCommandType09, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
        SET @Error = @@ERROR
...
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [olah].[IndexOptimize]
      ...
      BEGIN
       ...
       -- Is one of the columns in the index an image, text or ntext data type?
        ...
        BEGIN
          ...
          EXECUTE @CurrentCommandOutput13 = [olah].[CommandExecute] @Command = @CurrentCommand13, @CommandType = @CurrentCommandType13, @Mode = 2, @Comment = @CurrentComment, @DatabaseName = @CurrentDatabaseName, @SchemaName = @CurrentSchemaName, @ObjectName = @CurrentObjectName, @ObjectType = @CurrentObjectType, @IndexName = @CurrentIndexName, @IndexType = @CurrentIndexType, @PartitionNumber = @CurrentPartitionNumber, @ExtendedInfo = @CurrentExtendedInfo, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
        ...
        BEGIN
          ...
          EXECUTE @CurrentCommandOutput14 = [olah].[CommandExecute] @Command = @CurrentCommand14, @CommandType = @CurrentCommandType14, @Mode = 2, @DatabaseName = @CurrentDatabaseName, @SchemaName = @CurrentSchemaName, @ObjectName = @CurrentObjectName, @ObjectType = @CurrentObjectType, @IndexName = @CurrentIndexName, @IndexType = @CurrentIndexType, @StatisticsName = @CurrentStatisticsName, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
...
BEGIN
  ...
  SET @JobCategory = 'SQLOPs - Database Maintenance'
  SET @JobOwner = SUSER_SNAME(0x01)

  SET @JobName01 = 'SQLOPs - DatabaseBackup - SYSTEM_DATABASES - FULL'
  SET @JobCommand01 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[DatabaseBackup] @Databases = ''SYSTEM_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL') + ', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName02 = 'SQLOPs - DatabaseBackup - USER_DATABASES - DIFF'
  SET @JobCommand02 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''DIFF'', @Verify = ''Y'', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL') + ', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName03 = 'SQLOPs - DatabaseBackup - USER_DATABASES - FULL'
  SET @JobCommand03 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''FULL'', @Verify = ''Y'', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL') + ', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName04 = 'SQLOPs - DatabaseBackup - USER_DATABASES - LOG'
  SET @JobCommand04 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[DatabaseBackup] @Databases = ''USER_DATABASES'', @Directory = ' + ISNULL('N''' + REPLACE(@BackupDirectory,'''','''''') + '''','NULL') + ', @BackupType = ''LOG'', @Verify = ''Y'', @CleanupTime = ' + ISNULL(CAST(@CleanupTime AS nvarchar),'NULL') + ', @CheckSum = ''Y''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName05 = 'SQLOPs - DatabaseIntegrityCheck - SYSTEM_DATABASES'
  SET @JobCommand05 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[DatabaseIntegrityCheck] @Databases = ''SYSTEM_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName06 = 'SQLOPs - DatabaseIntegrityCheck - USER_DATABASES'
  SET @JobCommand06 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[DatabaseIntegrityCheck] @Databases = ''USER_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName07 = 'SQLOPs - IndexOptimize - USER_DATABASES'
  SET @JobCommand07 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "EXECUTE [olah].[IndexOptimize] @Databases = ''USER_DATABASES''' + CASE WHEN @LogToTable = 'Y' THEN ', @LogToTable = ''Y''' ELSE '' END + '" -b'
  ...
  SET @JobName08 = 'SQLOPs - sp_delete_backuphistory'
  ...
  SET @JobName09 = 'SQLOPs - sp_purge_jobhistory'
  ...
  SET @JobName10 = 'SQLOPs - Output File Cleanup'
  ...
  SET @JobName11 = 'SQLOPs - CommandLog Cleanup'
  SET @JobCommand11 = 'sqlcmd -E -S ' + @TokenServer + ' -d ' + @DatabaseName + ' -Q "DELETE FROM [olah].[CommandLog] WHERE StartTime < DATEADD(dd,-30,GETDATE())" -b'
  ...
  EXECUTE msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name = @JobCategory 
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName01, @name=N'Evening Backup', @enabled=1, @freq_type=4, @freq_interval=1, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=0, @active_start_time=195700
	--,@active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959 
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName02, @name=N'Daily Differential backup', @enabled=1, @freq_type=8, @freq_interval=119, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=195900
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName03, @name=N'Bi Weekly Backup', @enabled=1, @freq_type=8, @freq_interval=9, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=195900 
	--, @active_start_date=20151217, @active_end_date=99991231,	@active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName04, @name=N'Hourly', @enabled=1, @freq_type=4, @freq_interval=1, @freq_subday_type=8, @freq_subday_interval=1, @freq_relative_interval=0, @freq_recurrence_factor=0, @active_start_time=31500, @active_end_time=211559
	--, @active_start_date=20151217, @active_end_date=99991231
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName05, @name=N'Sunday Evening', @enabled=1, @freq_type=8, @freq_interval=1, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=193700
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName06, @name=N'DBCC User DBs', @enabled=1, @freq_type=8, @freq_interval=64, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=181700
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName07, @name=N'IndexOptimize', @enabled=1, @freq_type=8, @freq_interval=1, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=300
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName08, @name=N'Weekly Cleanup', @enabled=1, @freq_type=8, @freq_interval=64, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=171717
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName09, @name=N'Weekly Cleanup', @enabled=1, @freq_type=8, @freq_interval=64, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=171717 
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
	...
	EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName10, @name=N'Weekly Cleanup', @enabled=1, @freq_type=8, @freq_interval=64, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=171717
	--, @active_start_date=20151217, @active_end_date=99991231, @active_end_time=235959
  ...
  BEGIN
    ...
    EXECUTE msdb.dbo.sp_add_jobschedule @job_name = @JobName11, @name=N'Sunday Evening', @enabled=1, @freq_type=8, @freq_interval=1, @freq_subday_type=1, @freq_relative_interval=0, @freq_recurrence_factor=1, @active_start_time=193700, @active_end_time=235959
	-- @active_start_date=20151217, @active_end_date=99991231,	
