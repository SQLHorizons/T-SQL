# T-SQL
 ...
 ```powershell 
            SqlRS DefaultConfiguration
            {
                ...
                DatabaseInstanceName         = $AllNodes.InstanceName
                ReportServerVirtualDirectory = $AllNodes.VirtualDirectory
                ReportsVirtualDirectory      = $AllNodes.ReportsVirtualDirectory
                ReportServerReservedUrl      = @("http://+:80", "https://+:443")
                ReportsReservedUrl           = @("http://+:80", "https://+:443")
                DependsOn                    = "[SqlRSSetup]InstallReportingServices"
            }
```
